...
| <a name="input_iam_role_path"></a> [iam\_role\_path](#input\_iam\_role\_path) | IAM path applied to the task and execution roles when the module creates them (ignored when task\_role/execution\_role are supplied). Required by deploy roles that scope iam:CreateRole resources by path, e.g. `/services/<repo>/`. | `string` | `null` | no |
