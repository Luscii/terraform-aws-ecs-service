...
| <a name="output_volume_iam_policy_json"></a> [volume\_iam\_policy\_json](#output\_volume\_iam\_policy\_json) | The least-privilege IAM policy document (JSON) the module computes for the declared `volumes`. Already attached inline to the task role by default. Null when no volume contributes statements (e.g. ephemeral-only, attach\_iam\_policy=false everywhere, or EFS without IAM auth). Useful for consumers on bring-your-own task roles, or those who opted out of auto-attach via `attach_iam_policy = false` and want to attach the policy themselves. |
<!-- END_TF_DOCS -->
